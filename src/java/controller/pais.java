...
public class pais extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accion.equals("Registrar")) {
            ...
            if (length == 0) {
                ...
                if (r == 1) {
                    ...
                    request.setAttribute("Pais", p);
                    request.getRequestDispatcher("paisActivo.jsp").forward(request, response);
                } else {
                    limpiarcampos();
                    Variables.mod = 1;
        ...
        if (accion.equals("Cambiar Inactivo")) {
            ...
            p.setId_pais(codigo);
            r = pbd.cambiarInactivo(p);
            pais = leer.leerPaisIna();
            request.setAttribute("PaisL", pais);
            request.setAttribute("Pais", p);
            request.getRequestDispatcher("paisInactivo.jsp").forward(request, response);
        }
    ...
    private void limpiarcampos() {
        ...
        int id = 0;
        p.setDescripcion_pais(variable);
        p.setEstado_pais(variable);
        p.setId_pais(id);
    }
